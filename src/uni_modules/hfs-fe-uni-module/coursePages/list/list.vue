...
export default {
  ...
  async onLoad() {
    ...
    this.eventChannel.on('courseSend', (data) => {
  ...
  methods: {
    async getCourseList() {
      ...
      const res = await uni.$http.get(`${this.prefix}/api/v3/micro-course/search`, { ...this.searchData, pagesize, pageindex })
      this.courseList = this.courseList.concat(res.data || [])
    },
    async getVersion() {
      const res = await uni.$http.get(`${this.prefix}/tiku_api_proxy/v1/tiku-kecheng-version`, { grade: this.profile.grade, subject: this.subjectName, version: this.subjectVersion })
    ...
    profileChange(data) {
      ...
      this.eventChannel.emit('profileChange', data)
    ...
    versionClick() {
      let url = `/pages/edit/editInfo?subject=${this.subjectName}&back=true`
      // #ifdef MP-WEIXIN
    ...
    toDetail(item) {
      uni.navigateTo({
        url: `/uni_modules/hfs-fe-uni-module/coursePages/detail/detail?pack_id=${item._id}`,
      })
    },
    async resetDta() {
      this.pageindex = 1
