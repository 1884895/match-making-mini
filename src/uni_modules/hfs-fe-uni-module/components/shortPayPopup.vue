  ...
  <u-popup :show="props.show" mode="bottom" :overlay="props.overlay" :overlay-opacity="props.overlayOpacity" bg-color="transparent">
    <view class="payment-popup">
      ...
      <view class="payment-popup__card">
        ...
        <view class="price">
          <view class="price-day"><text class="active">{{ currentGoods.name }}</text>补贴价</view>
          <view class="price-cost"><text class="active">{{ currentGoods.cost }}<text v-if="recommendation.giftDays" class="gift">赠 {{ recommendation.giftDays }} 天</text></text>元</view>
          <view class="price-original">{{ currentGoods.original }} 元</view>
        </view>
        <view class="desc" :class="{ service: currentGoods.showServiceTo }">
          <text class="active">仅享 1 次</text>
          <template v-if="currentGoods.dayPrice && currentGoods.dayPrice < 0.99">，补贴后低至 <text class="bold">{{ currentGoods.dayPrice }}</text>  元/天</template>
          <text v-if="currentGoods.showServiceTo">，服务至{{ currentGoods.serviceTo }}</text>
        </view>
      ...
      <view class="payment-popup__btn">
        <view class="button" @click="paymentVIP">立即支付 {{ currentGoods.cost }} 元</view>
...
const emits = defineEmits(['update:show', 'payment', 'protocol', 'close'])
const resourceUrl = 'https://fe-resource.haofenshu.com/member/'

const currentGoods = computed(() => props.currentGoods || {})
const recommendation = computed(() => props.shortInfo?.recommendation || {})
