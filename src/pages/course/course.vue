<template>
  <view class="course">
    <u-navbar placeholder>
      <template #left>
        <view class="navbar-left" @click="handleBack">
          <u-icon name="arrow-left" size="19" />
        </view>
      </template>
      <template #center>
        <view class="navbar-center" @click="gradeClick">
          <view class="navbar-center__grade">
            {{ profile.grade || '选择年级' }}<u-icon name="arrow-down-fill" color="#333333" size="18rpx" :custom-style="{ marginLeft: '10rpx' }" />
          </view>
          <view class="navbar-center__text">
            版本教材
          </view>
        </view>
      </template>
    ...
    <view class="course-content">
      <AdSwiper height="234rpx" :list="adsList" key-name="picUrl" />
...
export default {
  ...
  onShow() {
    this.courseList = uni.getStorageSync('courseCache') || {}
  },
  methods: {
    // 获取智能营销广告
    getAdvert() {
      ...
      const params = {
        ...
        schoolid,
        slot_id: feConfig.courseBannerSlotId,
        user_tag: userTag,
        userid,
      }
    ...
    async getCourseOption() {
      const res = await uni.$http.get('/v1/be-proxy/api/v4/micro-course/pack/options')
    ...
    toDetail(item, index) {
      uni.navigateTo({
        url: `/tabPages/course/detail?pack_id=${item._id}&subject=${index}`,
      })
    },
    toStudy() {
      if (!this.isLogin)
