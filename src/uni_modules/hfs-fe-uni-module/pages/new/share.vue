<template>
  <view class="share">
    ...
    <view class="share-launch">
      <ILaunchApp>
        <view class="share-launch__content" @click="report('yyy_dkapp_click')">
    ...
    <view class="share-info">
      ...
      <view class="share-info__button flex-row-center">
        ...
        <!-- #ifdef H5 -->
        <ILaunchApp>
          <view class="btn flex-row-center" @click="report('yyy_ljxz_click')">
            立即下载
          </view>
        </ILaunchApp>
        <!-- #endif -->
    ...
    <view class="share-flow">
      <!-- #ifdef MP-WEIXIN -->
      <image class="share-flow__title bottom" :src="`${resourceUrl}share_title.png`" mode="widthFix" />
      <!-- #endif -->
      <!-- #ifdef H5 -->
      <image v-if="isHFSAppStudent" class="share-flow__title" :src="`${resourceUrl}share_title_student.png`" mode="widthFix" />
      <image v-else class="share-flow__title" :src="`${resourceUrl}share_title.png`" mode="widthFix" />
      <!-- #endif -->
      <!-- #ifdef H5 -->
      <ILaunchApp>
      ...
      <!-- #endif -->
      <!-- #ifdef H5 -->
      <image v-if="isHFSAppStudent" class="share-flow__detail" :src="`${resourceUrl}share_flow_student.png`" mode="widthFix" />
      <image v-else class="share-flow__detail" :src="`${resourceUrl}share_flow.png`" mode="widthFix" />
      <!-- #endif -->
      <!-- #ifdef H5 -->
      <ILaunchApp>
        <view class="share-flow__open flex-row-center" @click="report('yyy_dkhfs_click')">
    ...
    <view class="share-tips">
      ...
      <view class="share-tips__content">
        1.完成绑定就能查到成绩啦   2.绑定时记得输入邀请码领{{ isHFSAppStudent ? '双' : '三' }}重绑定礼～
      </view>
    </view>
    <view class="share-footer">
      ...
      <!-- #ifdef H5 -->
      <ILaunchApp>
        <view class="share-footer__button flex-row-center" @click="report('yyy_ljxz_click')">
          立即下载<image class="run" :src="`${resourceUrl}operation_run.png`" mode="widthFix" />
        </view>
      </ILaunchApp>
      <!-- #endif -->
    </view>
...
export default {
  ...
  methods: {
    ...
    handleCopy() {
      ...
      this.report('yyy_fzlq_click')
    },
    report(action) {
      report(this.actionId, action)
    },
  },
}
</script>
