  ...
  methods: {
    ...
    async init() {
      ...
      if (this.pageType === 'share')
        return
      return await this.getInvitation()
    },
    async getInvitation() {
      try {
        const params = {}
        this.pageType === 'operation' && (params.onlyCode = 1)
        const res = await uni.$http.get(`${this.prefix}/v3/new-promoter/invitation`, params, { custom: { error: false } })
        this.inviterInfo = res.data || {}
        uni.setNavigationBarTitle({ title: this.inviterInfo.name || '校园新推官' })
        this.initPage()
      }
      catch (err) {
    ...
    async getInvitationCode() {
      ...
      const { data } = await uni.$http.post(`${this.prefix}/v3/new-promoter/invitation-code`, params, { custom: { loading: false, error: false } })
    ...
    async getShareWeixinInfo() {
      this.openInvite()
      return {
        title: '点击链接注册绑定好分数，就能￥0.99领周卡会员',
        content: '4500万学生的智慧学习方式！现在下载更有会员体验券包、大额优惠券、老用户同等邀请福利0元送！',
        path: `/uni_modules/hfs-fe-uni-module/pages/newPromoter/share?icode=${await this.getInvitationCode()}`,
    ...
    async shareWeixin() {
      ...
      const qrUrl = `${window.location.origin}/#/uni_modules/hfs-fe-uni-module/pages/newPromoter/share${uni.$u.queryParams({ icode: await this.getInvitationCode(), platform })}`
    ...
    async getShareRemindInfo() {
      this.openInvite()
      return {
        title: `${this.studentInfo.studentName}${this.userInfo.roleType === 2 ? '家长' : ''}提醒你去绑定！绑定才能查成绩+0.99元购周卡！`,
        content: '4500万学生的智慧学习方式！现在下载更有会员体验券包、大额优惠券、老用户同等邀请福利0元送！',
        path: `/pages/index/index`,
        imageUrl: `${this.resourceUrl}weixin_share_remind.png`,
      }
    },
    async showRemindCopy() {
      const title = '点击链接注册绑定好分数，就能￥0.99领周卡会员: '
      const url = `${this.inviterInfo.XCXPath}${encodeURIComponent(uni.$u.queryParams({ icode: await this.getInvitationCode() }, false))}`
    ...
    async getShareCopyInfo() {
      ...
      // #ifdef H5
      const platform = isHFSStudent() ? 'student' : 'parent'
      params.title = '点击链接注册绑定好分数，就能￥0.99领周卡会员: '
      params.url = `${window.location.origin}/#/uni_modules/hfs-fe-uni-module/pages/newPromoter/share${uni.$u.queryParams({ icode: await this.getInvitationCode(), platform })}`
      // #endif
      // #ifdef MP-WEIXIN
      params.title = '点击链接注册绑定好分数，就能￥0.99领周卡会员: '
    ...
    async getAwardList() {
      const { data = [] } = await uni.$http.get(`${this.prefix}/v3/new-promoter/award`, {}, { custom: { loading: false, error: false } })
      this.awardList = data
    },
    openInvite() {
      setTimeout(() => {
        this.$refs.popup.open('invite')
      }, 300)
    },
  },
}
