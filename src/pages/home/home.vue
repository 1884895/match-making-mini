  ...
  <view class="home">
    <view class="home-content">
      <u-navbar placeholder>
        <template #left>
          <view class="info" @click="gradeClick">
            <image class="info-avatar" src="../../static/home/mine_img_avatar_default.png" />
            <template v-if="isLogin">
              <view v-if="version" class="info-area">
                {{ version }}<u-icon name="arrow-down-fill" color="#333333" size="18rpx" :custom-style="{ marginLeft: '10rpx' }" />
              </view>
              <view v-else class="info-area">
                设置教材版本
              </view>
            </template>
            <template v-else>
        ...
        <template #right>
          <view class="customer" :style="menuButtomWidth" @click="connectCustomer">
            <view class="customer-tem">
              <image class="customer-tem-avatar" src="../../static/me/mine_icon_khfw.png" />
              <view class="customer-tem-text">
                客服
              </view>
            </view>
          </view>
      ...
      <view class="type">
        <view v-for="item in typeList" :key="item.title" class="type-item" :style="{ background: item.style.background }" @click="jumpWebView(item.type)">
          <view class="type-item__content">
            <view class="title">
              {{ item.title }}
            </view>
            <view class="subtitle">
      ...
      <view class="tools tools-study">
        <view class="tools-title">
          <view class="line" />学习
        </view>
        <view class="tools-list">
          <template v-for="item in studyList">
            <view v-if="item.show" :key="item.title" class="tools-list-item" @click="jumpWebView(item.type)">
              <image v-if="item.topIcon" class="hot" :src="resourceUrl + item.topIcon" />
              <image class="icon" :src="resourceUrl + item.icon" />
              <view class="title">
                {{ item.title }}
              </view>
      ...
      <!-- 运营区 -->
      <Operation :operation-detail="operationDetail" />
    </view>
    <image v-if="configControl.inviteShow" class="home-btn" :src="`${resourceUrl}invite-gif.gif`" mode="widthFix" @click="jumpWebView('invite')" />
    <ShortPopup v-model:show="shortInfo.show" :short-info="shortInfo" :raise-info="raiseInfo" @abandon="shortInfo.show = false" @close="shortInfo.show = false" @payment="shortInfo.show = false" />
    <UnpaidPopup v-model:show="showUnpaid" :degrade-info="degradeInfo" @close="showUnpaid = false" />
    <u-popup :show="popup.show" :mode="popup.mode" :overlay="popup.overlay" bg-color="transparent" @touchmove.stop.prevent>
      <!-- 待支付订单 -->
      <template v-if="popupInfo.type === 1">
        <view class="unpaid">
          <image class="unpaid-note" :src="`${memberUrl}popup_note.png`" mode="widthFix" />
          <image class="unpaid-order" :src="`${memberUrl}popup_order.png`" mode="widthFix" />
          <view class="unpaid-info">
            <view class="unpaid-info__content">
              <view class="title">
                {{ noPayOrder.goodName }}
              </view>
              <view class="desc">
                <text>
                  <text class="active">
                    {{ noPayOrder.amount }}
                  </text>元<template v-if="discount">
                    丨已减<text class="active discount">
                      {{ discount }}
                    </text> ！
                  </template>
                </text>
            ...
            <image class="unpaid-info__bg" :src="`${memberUrl}popup_unpaid.png`" mode="widthFix" />
          </view>
          <view v-if="noPayOrder.remainSec" class="countdown">
            <image class="countdown-icon" :src="`${memberUrl}popup_clock.png`" />
            <u-count-down :time="noPayOrder.remainSec || 0" format="HH:mm:ss" auto-start @change="countChange" @finish="countFinish">
              <view class="count">
          ...
          <view class="unpaid-button" @click="continuePay">
            继续支付
          </view>
          <image class="unpaid-close" :src="`${memberUrl}popup_close.png`" @click="popup.show = false" />
        </view>
      </template>
      <!-- 优惠券 -->
      <template v-if="popupInfo.type === 2">
        <view class="coupon">
          <image class="coupon-crown" :src="`${memberUrl}popup_crown.png`" mode="widthFix" />
          <image class="coupon-ticket" :src="couponInfo.receiveNow === 1 ? `${memberUrl}popup_time.png` : `${memberUrl}popup_ticket.png`" mode="widthFix" />
          <view class="coupon-info">
            <view class="coupon-info__content">
              <view class="price">
                <text>
                  ¥<text class="active">
              ...
              <view class="text">
                ...
                <view class="text-tips">
                  {{ couponInfo.promotionText }}
                </view>
              </view>
            </view>
            <image class="coupon-info__bg" :src="`${memberUrl}popup_gift.png`" mode="widthFix" />
          </view>
          <view v-if="couponInfo.remainTime" class="countdown">
            <image class="countdown-icon" :src="`${memberUrl}popup_clock.png`" />
            <u-count-down :time="(couponInfo.remainTime || 0) * 1000" format="HH:mm:ss" auto-start @change="countChange" @finish="countFinish">
              <view class="count">
                <view class="count-item">
                  {{ formatNumber((timeData.hours + timeData.days * 24) || 0) }}
                </view>
          ...
          <view class="unpaid-button" @click="clickVip">
            使用优惠
          </view>
          <image class="unpaid-close" :src="`${memberUrl}popup_close.png`" @click="popup.show = false" />
        </view>
      ...
      <!-- 会员倒计时 -->
      <template v-if="popupInfo.type === 3">
        <view class="renew">
          <view class="renew-info">
            <view class="renew-info__title">
              {{ popupInfo.renew.title }}
            </view>
            <view class="renew-info__desc">
              {{ popupInfo.renew.subTitle }}
            </view>
            <view class="renew-info__content">
              <view class="content-title">
                {{ popupInfo.renew.good }}<image class="vip" :src="`${resourceUrl}renew_vip.png`" />
              </view>
              <image v-if="popupInfo.renew.remainDay > 0" class="content-bg" style="margin-bottom: 30rpx;" :src="`${resourceUrl}renew_guide.png`" mode="widthFix" />
              <image v-else class="content-bg" style="margin-bottom: 10rpx;" :src="`${resourceUrl}renew_guide_large.png`" mode="widthFix" />
            </view>
            <view class="renew-info__button" @click="clickVip">
              <image class="button" :src="`${resourceUrl}${popupInfo.renew.remainDay > 0 ? 'renew_pay.png' : 'renew_open.png'}`" mode="widthFix" />
            </view>
          </view>
          <image class="close" src="../../static/home/button_close.png" @click="popup.show = false" />
          <image class="renew-bg" :src="`${resourceUrl}renew_bg.png`" mode="widthFix" />
        </view>
      </template>
    </u-popup>
  </view>
...
export default {
  components: { Operation, UnpaidPopup, ShortPopup },
  data() {
    return {
